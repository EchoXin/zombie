...
/***/ function(module, exports) {
	...
	var app = {
	  api: 'https://agile-brushlands-41784.herokuapp.com'
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var onCreate = function onCreate() {
	  api.create().done(ui.success).fail(ui.onError);
	};
	...
	var characterscore = document.getElementById('characterscore');
	var scores = 0;
	var set = void 0;
	var zombieSet = void 0;

	// character object
	var character = function character(hp, X, Y, sizeX, sizeY, score, dietime, speed, imagesrc) {
	  this.characterX = X;
	  this.characterY = Y;
	  this.imagenode = null;
	  this.characterhp = hp;
	...
	// fire object
	var fire = function fire(X, Y, sizeX, sizeY, imagesrc) {
	  this.fireX = X;
	  this.fireY = Y;
	  this.fireimage = null;
	  this.fireattach = 1;
	...
	// create fire
	var Oddfire = function Oddfire(X, Y) {
	...
	var Zombie = function Zombie(hp, a, b, sizeX, sizeY, score, dietime, speed, imagesrc) {
	...
	// create self pokemon
	var selfPokemon = new Pokemon(284, 700);

	var livePokemon = document.getElementById('pokemon');
	var mouseMove = function mouseMove() {

	  var oevent = window.event || arguments[0];
	  var selfPokemonX = oevent.clientX;
	  var selfPokemonY = oevent.clientY - 30;
	...
	var start = function start() {
	  ...
	  if (mark === 30) {
	    mark2++;
	    var a = mainDiv.clientTop;
	    var b = a + mainDiv.clientHeight - 70;
	    var c = a + mainDiv.clientHeight - 150;

	    zombies.push(new Zombie(5, a, b, 60, 70, 1, 360, random(1, 5), 'assets/styles/image/zombie.gif'));

	    if (mark2 % 10 === 0) {
	      var zombieBoss = new Zombie(10, a, c, 130, 150, 3, 2000, random(1, 5), 'assets/styles/image/zombie-boss.gif');
	  ...
	  // zombies move
	  var zombieslen = zombies.length;
	  for (var i = 0; i < zombieslen; i++) {
	  ...
	  // move fire
	  var fireslen = fires.length;
	  for (var _i = 0; _i < fireslen; _i++) {
	    fires[_i].firemove();
	    if (fires[_i].fireimage.offsetTop < 0) {
	      mainDiv.removeChild(fires[_i].fireimage);
	      fires.splice(_i, 1);
	      fireslen--;
	    }
	  }

	  for (var k = 0; k < fireslen; k++) {
	    for (var j = 0; j < zombieslen; j++) {
	      //collision test
	...
	var begin = function begin(event) {
	  event.preventDefault();
	  $('.fire').remove();
	  $('.zombie').remove();
	  $('.zombie-boss').remove();
	...
	//game over
	var onGetRanking = function onGetRanking() {
	  console.log('show');
	  api.getGames().done(api.displayRanking).fail(ui.onError);
	};

	var showGameHistory = function showGameHistory(data) {
	  var total = 0;
	  var highest = 0;
	  var lowest = 0;
	  if (data.games[0]) {
	...
	var GameHistory = function GameHistory() {
	  api.getMyGames().done(function (data) {
	    return showGameHistory(data);
	  }).fail(ui.onError);
	};

	var OnCleanHistory = function OnCleanHistory(event) {
	  event.preventDefault();
	  api.cleanHistory().done(ui.success).fail(ui.failure);
	  $('#total-games').html('0');
...
/***/ function(module, exports, __webpack_require__) {
	...
	var displayRanking = function displayRanking(data) {
	  $('#ranking-list').html('');
...
/***/ function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__.p + "9696c9935824772090aab27c3b45ae35.jpg";

...
// fire object
function fire(X,Y,sizeX,sizeY,imagesrc){
    this.fireX=X;
    this.fireY=Y;
    this.fireimage=null;
    this.fireattach=1;
...
// create fire
function oddfire(X,Y){
    fire.call(this,X,Y,6,14,'assets/scripts/image/fire.png');
}


// create zombie
function zombie(hp,a,b,sizeX,sizeY,score,dietime,sudu,imagesrc){
...
function pokemon(X,Y){
    ...
    this.imagenode.setAttribute('id','live-pokemon');
}

// create self pokemon
let selfPokemon=new pokemon(284,700);

let livePokemon=document.getElementById('live-pokemon');
let yidong=function(){
...
function start(){
    ...
    if(mark==20){
            ...
            zombies.push(new zombie(6,25,500,80,100,5000,360,random(1,3),'assets/scripts/image/zombie.gif'));
        mark=0;
    }
